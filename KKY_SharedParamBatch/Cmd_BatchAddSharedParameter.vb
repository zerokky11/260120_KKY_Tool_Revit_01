        ...
        Private Shared Function ResolveCategoryInDoc(maps As CategoryMaps, cref As CategoryRef, ByRef resolvedBy As String) As Category
            ...
            If path <> "" Then
                If maps.ByPath.TryGetValue(path, cat) AndAlso cat IsNot Nothing Then
                    ' --- STAIRS SUPPORT FIX: if this path resolves to Supports, substitute to bindable category
...
#Region "Categories Tree Build (SubCategories + Path)"
        ...
        Private Shared Function BuildCategoryTreeItemRecursive(cat As Category,
            ...
            ' --- STAIRS SUPPORT FIX: show Stairs: Supports in selection UI (it will be substituted at bind time)
            Try
        ...
        Private Shared Function BuildAvailableCategoryMaps(doc As Document) As CategoryMaps
            ...
            ' --- STAIRS SUPPORT FIX: substitute Supports category to a bindable category for binding
            Try
        ...
        Private Shared Sub InjectStairsSupportsBindingFix(doc As Document, maps As CategoryMaps)
            ...
            ' Use a Stringer/Carriage ElementType to obtain a stable Category object
            ' (Some stair-related categories are problematic via Settings.Categories.get_Item in certain environments.)
            ...
            ' Map Supports-id to the replacement category (so ID-based resolution works)
            Try
            ...
            ' If this doc exposes a Supports path entry, redirect it too
            Try
